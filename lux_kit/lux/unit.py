...
class Unit:
    ...
    def transfer(
        self, transfer_direction, transfer_resource, transfer_amount, repeat=0
    ):
        assert transfer_resource < 5 and transfer_resource >= 0
        assert transfer_direction < 5 and transfer_direction >= 0
        return np.array(
            [1, transfer_direction, transfer_resource, transfer_amount, repeat]
        )
    ...
    def self_destruct_cost(self, game_state):
        ...
        weather_cfg = get_weather_config(current_weather, self.env_cfg)
        return math.ceil(
            self.unit_cfg.SELF_DESTRUCT_COST * weather_cfg["power_loss_factor"]
        )
